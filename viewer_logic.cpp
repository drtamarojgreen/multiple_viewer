...
void renderNexusFlow(Graph& graph) {
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        for (auto& node : graph.nodes) {
            ...
            positions[node.index].x = std::max(0.0f, std::min(positions[node.index].x, static_cast<float>(CONSOLE_WIDTH - 1)));
            positions[node.index].y = std::max(0.0f, std::min(positions[node.index].y, static_cast<float>(CONSOLE_HEIGHT - 1)));
        }
    }
