# SORREL: Standardizing of Resilient and Reliable Equipment Learning

This document summarizes the core principles, concepts, and architectural decisions made for the SORREL project, developed through Sorrel Driven Development (SDD).

## 1. What SORREL Is (and Isn't)

SORREL is a **development structure** designed to make AI agents **useful collaborators for humans** by aligning AI strengths (generation, adaptation) with software engineering realities (long-term context, architectural discipline, verified execution).
...
## 2. Sorrel Driven Development (SDD) Philosophy
...
-   **Metaphor:** Taking small sips of hot sorrel to prevent burning (speculative code).
...
## 7. SORREL Restrictions Engine (CRE)

The structural enforcement layer of SDD, preventing AI agents from generating unverified code through incremental units.
...
## 8. SORREL Reasoning Gate System (RGS)
...
-   **Allowed:** Discover -> Adapt -> Operate (dynamic location of `sdd` directory, `facts` directory).
-   **Philosophy:** Generated code describes an idea; organic code survives in an environment.

## 10. The SORREL Discovery Rule
...
## 11. Minimal SDD Repository Structure (within `tests/sorrel/`)
...
└── sdd/
    ...
    ├── sorrel_checkouts.md  (Completed work history)
    ├── facts/
...
## 12. SORREL CLI Capabilities (Implemented Sips)

-   `sorrel`: Prints "SORREL CLI".
-   `sorrel sip`: Prints "SIP RULES".
-   `sorrel discover sdd`: Dynamically locates and reports `sdd_directory_path`.
-   `sorrel discover facts`: Dynamically locates and reports `facts_directory_path`.
...
SORREL is not about making AI more powerful, but **more disciplined**. It transforms AI from a high-confidence generator into a disciplined engineer operating under strict supervision, producing **organic code that survives in an environment.**
...
SDD's profitability lies in solving existing business problems by **verifying reality**. The product is not SDD as a philosophy, but the tangible value it provides.
