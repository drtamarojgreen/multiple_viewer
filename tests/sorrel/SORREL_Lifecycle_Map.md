...
graph TD
    ...
    subgraph SORREL System
        ...
        CHAI_CHECK[Update sorrel_checkins.md (Future Work)]
    end
...
4.  **SORREL Reasoning Gate System (CHAI_RGS):** This acts as a checkpoint. It validates the AI's proposed plan against the Situation's constraints and the SORREL Discovery Rule.
...
5.  **SORREL Restrictions Engine (CHAI_CRE):** Enforces strict sip constraints (e.g., one capability, no placeholders, no hardcoded paths).
...
7.  **SORREL Card Runner (CHAI_RUN):** The system's runner takes the generated **Card (CARDS)** and executes it against the **Environment (CHAI_ENV)**.
8.  **Observe Results (CHAI_OBS):** The SORREL system observes and captures the empirical output of the Card's execution, forming **Card Results (RESULTS)**.
9.  **Record Fact (CHAI_REC):** If the **Card Results (RESULTS)** verify a new piece of knowledge, the SORREL system records this as an immutable entry in **Facts (FACTS)**.
10. **Update Checkins (CHAI_CHECK):** For any conceptual or unimplemented parts of the work, the `sorrel_checkins.md` (**CHECKINS**) file is updated.
